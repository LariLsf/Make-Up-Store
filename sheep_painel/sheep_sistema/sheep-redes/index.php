...
$lerUp = new Ler();
$lerUp->Leitura('redes_sociais', "WHERE id = :id", "id={$mostraId}");
if($lerUp->getResultado()){
foreach($lerUp->getResultado() as $redeUp);  
$redeUp = (object) $redeUp;
}
                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            ...
                            <div class="form-group row mb-4">
                                ...
                                <div class="col-md-4">
                                    <input type="text" class="form-control" name="icone" placeholder="Icone" value="<?= $redeUp->icone ? $redeUp->icone : null; ?>" style="border: 1px solid red;">
                                </div>
                                <div class="col-md-3">
                                    <input type="text" class="form-control" name="nome" placeholder="Nome" value="<?= $redeUp->nome ? $redeUp->nome : null; ?>" style="border: 1px solid red;">
                                </div>
                            ...
                            <div class="form-group row mb-4">
                                <label class="col-form-label text-md-right col-12 col-md-3 col-lg-3">Link(Obrigatório)</label>
                                <div class="col-md-7">
                                    <input type="url" class="form-control" name="link" placeholder="Link" value="<?= $redeUp->link ? $redeUp->link : null; ?>" style="border: 1px solid red;">
                                </div>

                                <input type="hidden" name="id" value="<?= $redeUp->id ?>">
                                <input type="hidden" name="tipo" value="redesSociais">
                    ...
                    <div class="card-body">
                        <div class="table-responsive">
                            <table class="table table-striped table-hover" id="save-stage" style="width:100%;">
                                ...
                                <tbody>
<?php 
$sheep->Leitura('redes_sociais', "ORDER BY data DESC");
$redesSociais = Formata::Resultado($sheep);
if($redesSociais){
foreach($sheep->getResultado() as $rede){
$rede = (object) $rede;

$escondeId = base64_encode($rede->id);
?>
<tr>
<td><?= $rede->id ?></td>
<td><?= date('d/m/Y', strtotime($rede->data)) ?></td>
<td><i class="<?= $rede->icone ? $rede->icone : null; ?>" style="font-size: 22px;"></i></td>
<td><a href="<?= $rede->link ? $rede->link : null;  ?>" title="<?= $rede->nome ? $rede->nome : null;  ?>"><?= $rede->nome ? $rede->nome : null;  ?></a> </td>
<td><a href="<?= URL_CAMINHO_PAINEL . FILTROS ?>sheep-redes/index&editar=<?= $escondeId ?>&token=<?=$_SESSION['timeWT']?>" class="btn btn-icon btn-primary"><i class="far fa-edit"></i></a></td>
<td>
...
<button type="submit" class="btn btn-icon btn-danger" onclick="return confirm('Você deseja realmente excluir')"><i class="fas fa-trash-alt"></i></button>
</form>
</td>
</tr>
<?php } } ?>
